...
const origin = (process.env.SITE_ORIGIN || process.env.NEXT_PUBLIC_SITE_ORIGIN || 'https://boutique-de-boxe.vercel.app').replace(/\/$/, '');
const host = new URL(origin).host;

let urls = process.argv.slice(2).map((p) => (p.startsWith('http') ? p : origin + p));
if (!urls.length) {
...
for (let i = 0; i < urls.length; i += 10000) {
  ...
  const r = await fetch('https://api.indexnow.org/indexnow', {
    ...
    body: JSON.stringify({ host, key: KEY, keyLocation: `${origin}/${KEY}.txt`, urlList: batch }),
  });
