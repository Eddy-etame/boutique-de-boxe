...
Contrôles locaux : `pnpm lint`, `pnpm build`, `node scripts/audit-catalog.mjs`, `node --test scripts/test-inlett.mjs scripts/test-payplug-orders.mjs`, `node --experimental-strip-types scripts/test-payplug.mjs`, `python scripts/test-payplug-sqlite.py`. Avec un serveur local et les e-mails désactivés : `node scripts/test-commerce.mjs --email-disabled` et `node scripts/audit-seo.mjs`.
