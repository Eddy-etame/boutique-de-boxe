...
1. **Domaine** : attacher le domaine définitif sur Vercel, poser `NEXT_PUBLIC_SITE_ORIGIN` et `PAYPLUG_PUBLIC_BASE_URL` dessus (canonicals, sitemap, hreflang, graphe, `llms.txt`, redirection de l’alias suivent).
...
- **Migration de domaine en cours (hors de cette passe).** Depuis le commit `26aa80e` (16 sept.), `shop.origin` est fixé à `https://boutique-de-boxe.fr` et l’alias `boutique-de-boxe.vercel.app` redirige en 308 vers ce domaine. Or `boutique-de-boxe.fr` pointe encore chez OVH (213.186.33.5), pas Vercel : le site public est donc injoignable par ces URL tant que le domaine n’est pas rattaché au projet Vercel et le DNS repointé. À finir côté hébergement (ajouter le domaine dans Vercel, pointer les enregistrements DNS d’OVH vers Vercel). Le code de cette passe est vérifié en local (lint, tsc, API 24, commerce 18, PayPlug 19+35, audit 1 104 pages) et les en-têtes de sécurité sont confirmés en ligne.
